...
| ▶️ 播放 | H.264/WebM 原生播放（HTTP Range）；**MKV/HEVC 实时转码为多档自适应 HLS**（可切换清晰度）；字幕自动识别（SRT→WebVTT）、内嵌字幕提取、字幕上传、**多语言切换**与按用户字幕偏好；支持下载 |
...
- [ ] 浏览器分片、可续传上传，带队列的上传管理器
- [ ] 下载为 MP4/MKV，可勾选音轨/字幕轨（无需重编码）
- [ ] 集成 yt-dlp：定时抓取在线网站视频/频道入库
...
- [ ] 本地语音转写（Whisper）→ 可搜索的字幕/文稿
- [ ] 可插拔元数据源 / 自定义刮削器，支持单条覆盖
