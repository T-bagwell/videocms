...
| ▶️ 播放 | H.264/WebM 原生播放（HTTP Range）；**MKV/HEVC 实时转码为多档自适应 HLS**（可切换清晰度）；字幕自动识别（SRT→WebVTT）、内嵌字幕提取、字幕上传、**多语言切换**、按用户偏好与 ASS 样式；**片头/片尾跳过**；**一起看**、**AirPlay/Chromecast 投屏**与 **DLNA**；预览缩略图；**下载为 MKV/MP4，可选音轨与字幕（转封装、不重编码）** |
...
| 🔐 单点登录 | **OIDC** 与 **SAML 2.0**（ADFS、Okta、Keycloak 等），SAML 属性可自动映射管理员角色 |
| 📡 直播与社交 | **RTMP 直播**（内置聊天）；视频**评论与评分**；**Whisper 语音转写**与 **AI 打标**；Webhook/Apprise/**邮件**通知 |
| 📦 运维 | **存储池**（本地/S3/SFTP）、定时**维护/备份**、任务看板、健康检查与重复清理、NFO 导入/导出、支持**离线保存**的 PWA |
...
- [x] 用户自定义标签、智能合集、保存的筛选条件
- [x] 标题/简介/文稿/标签的全文与模糊搜索
- [x] 批量整理（移动、重命名、批量打标）+ 回收站
- [x] NFO 元数据导入/导出，兼容 Plex/Jellyfin/Kodi
...
- [x] Webhook + 完善的公开 REST API，便于第三方自动化
- [x] PWA 离线下载与移动端体验打磨
...
- [x] 多版本影片：同一影片的多个文件（1080p/4K/加长版等）自动归组，播放时
      默认选择最佳版本，详情页可手动切换
- [x] 预告片与花絮：刮削时拉取官方预告片入库，也支持手动挂载自托管花絮
- [x] 影片/剧集主题曲：抓取或上传主题曲，详情页提供试听入口
- [x] 章节：从 ffprobe/章节文件读取章节，在播放器时间轴展示，并提供通用
      媒体片段 API
- [x] 更多转码档（AV1/VP9）与 HDR 直通：支持 Dolby Vision / HDR10+ 内容
      直通播放
- [x] 播放器增强：倍速、画中画、完整键盘快捷键
- [x] 按设备直连播放策略：不同设备/网络可配置直连或转码偏好，异常时自动兜底

**媒体类型与媒体库**

- [x] 音乐库：扫描音频文件，按艺术家/专辑组织，支持内嵌封面与无缝播放
- [x] 有声书：章节导航与播放速度记忆
- [x] 电子书与漫画：EPUB 阅读器与 CBZ 漫画阅读器
- [x] 照片库：相册、EXIF 元数据与幻灯片
- [x] 音频/图片/PDF 作为一等媒体类型入库展示
- [x] 逐条可见性：公开/私有/不公开/密码保护
...
- [x] 点播请求工作流：用户搜索并请求标题，管理员审批后接入下载队列
- [x] 质量配置与自动升级：优先最佳匹配文件，新版本自动升级，入库时自动
      移动/重命名
- [x] 更多可插拔元数据源（TVDB、AniDB、Fanart.tv、OMDb、Trakt 等），按库
      配置优先级
- [x] 海报之外的背景图/fanart/季封面
- [x] 观看历史同步到外部追踪服务（Trakt、SIMKL）
- [x] 更多字幕源，支持模糊匹配与手动覆盖

**用户、社交与分析**

- [x] 观看统计看板：播放次数、用户、设备、观看时长、图表与导出
- [x] 频道与订阅：用户频道页、关注按钮与「新内容」动态
- [x] 审核工具：内容举报、账号屏蔽、全局黑白名单与批量用户操作
- [x] 注册策略：开放/邀请制/关闭，支持邀请码
- [x] 点赞/点踩与逐项策略开关（下载、评论、举报）
- [x] 可筛选的动态流与用户级通知偏好
...
- [x] 分布式转码队列：优先级与工作节点扩缩
- [x] yt-dlp 增强：质量预设、代理/cookies/登录、频道/播放列表订阅抓取
- [x] 远程访问辅助：TURN/WebRTC 兜底与自动 TLS 证书
