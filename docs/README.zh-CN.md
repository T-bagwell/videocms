...
- [ ] IPTV 频道：从 M3U 源或媒体库生成定时频道，输出 M3U 与 EPG（XMLTV）
- [ ] 调谐器直播（DVB/ATSC/HDHomeRun）：节目指南与录像排程
- [ ] 台标、回看与时移播放

**发现、请求与自动化**

- [ ] 点播请求工作流：用户搜索并请求标题，管理员审批后接入下载队列
- [ ] 质量配置与自动升级：优先最佳匹配文件，新版本自动升级，入库时自动
      移动/重命名
- [ ] 更多可插拔元数据源（TVDB、AniDB、Fanart.tv、OMDb、Trakt 等），按库
      配置优先级
- [ ] 海报之外的背景图/fanart/季封面
- [ ] 观看历史同步到外部追踪服务（Trakt、SIMKL）
- [ ] 更多字幕源，支持模糊匹配与手动覆盖

**用户、社交与分析**

- [ ] 观看统计看板：播放次数、用户、设备、观看时长、图表与导出
- [ ] 频道与订阅：用户频道页、关注按钮与「新内容」动态
- [ ] 审核工具：内容举报、账号屏蔽、全局黑白名单与批量用户操作
- [ ] 注册策略：开放/邀请制/关闭，支持邀请码
- [ ] 点赞/点踩与逐项策略开关（下载、评论、举报）
- [ ] 可筛选的动态流与用户级通知偏好
