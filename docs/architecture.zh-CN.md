...
| `TMDB_API_KEY` / `TMDB_LANGUAGE` / `OMDB_API_KEY` / `FANART_API_KEY` / `TRAKT_CLIENT_ID` / `TRAKT_ACCESS_TOKEN` / `METRICS_ENABLED` / `OTEL_EXPORTER_OTLP_ENDPOINT` | 空 / zh-CN / 空 / 空 / 空 / 空 / `1` / 空 | 刮削（TMDB + OMDb + Fanart.tv）、Trakt 同步、Prometheus /metrics 与 OTLP 追踪 |
