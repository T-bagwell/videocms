...
- マルチ音声ソース：各音声ストリームは専用の AAC HLS トラック（`a<インデックス>/`）
...
- スタイル付き ASS 字幕：`.ass/.ssa` トラックは `format` フィールドを持ち、
...
- 検索：タイトル/概要/ファイル名/ジャンル/文字起こしの部分一致は pg_trgm の
  GIN インデックス（マイグレーション 023）で高速化。`sort=fuzzy` はフィルターを
  トライグラム類似度 > 0.15 に切り替え、そのスコアで並べます
...
| `TMDB_API_KEY` / `TMDB_LANGUAGE` / `OMDB_API_KEY` / `FANART_API_KEY` / `TRAKT_CLIENT_ID` / `TRAKT_ACCESS_TOKEN` / `METRICS_ENABLED` / `OTEL_EXPORTER_OTLP_ENDPOINT` | 空 / zh-CN / 空 / 空 / 空 / 空 / `1` / 空 | スクレイピング（TMDB + OMDb + Fanart.tv）、Trakt 同期、Prometheus /metrics と OTLP トレース |
