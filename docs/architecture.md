...
| `TMDB_API_KEY` / `TMDB_LANGUAGE` / `OMDB_API_KEY` / `FANART_API_KEY` / `TRAKT_CLIENT_ID` / `TRAKT_ACCESS_TOKEN` / `METRICS_ENABLED` / `OTEL_EXPORTER_OTLP_ENDPOINT` | empty / zh-CN / empty / empty / empty / empty / `1` / empty | Scraping (TMDB + OMDb + Fanart.tv), Trakt sync, Prometheus /metrics + OTLP traces |
